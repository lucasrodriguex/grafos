...
public class GrafosUtil {
	 ...
	 * @throws IOException
	 */
	public static void geraDistribuicaoEmpirica(Grafo grafo, String caminho) throws IOException {
	...
	/**
	 * Gera um arquivo txt com os caminhos percorridos pelo Bfs a partir
	 * de um vertice inicial
	 * @param grafo
	 ...
	 */
	public static void geraArvoreDeBusca(Grafo grafo, String caminho, int verticeInicial) {
	...
	public static void geraTxtSaida(Grafo grafo, String caminho){
        ...
        int maiorComponente = getMaiorComponenteConexo(grafo, componentesConexos);
		...
		try {
			...
			pw.println("Numero de componentes conexos: "+ numeroComponentesConexos);
	...
	public static List<BigDecimal> getDistribuicaoEmpirica(Grafo grafo){ 
		...
		for(int i = 0 ; i < grafo.getListaAdjacencias().size() ; i ++){
			divisao = (double) grafo.grauVertice(i) / (double) grafo.getNumeroVertices();
	...
	public static List<BigDecimal> getDistribuicaoEmpiricaDistancia(Grafo grafo){ 
		...
		double divisao;
		for(int i = 0 ; i < grafo.getListaAdjacencias().size() ; i ++){
			divisao = (double) grafo.grauVertice(i) / (double) grafo.getNumeroArestas();
			resultado = new BigDecimal(divisao);
			distribuicaoEmpirica.add(resultado);
		}
		return distribuicaoEmpirica;
	 ...
	 * @param caminho
	 * 			Caminho do arquivo que sera salvo em disco com a lista de adjacencias
	 * @throws IOException
	 */
	public static void printList(Map<Integer, LinkedList<Integer>> lista, String caminho)
			throws IOException {
		File f = new File(caminho);
		PrintWriter pw = new PrintWriter(f);
		...
		pw.close();
		System.out.println("Lista de adjacencias gerada em disco com sucesso...");
	}
	 ...
	 * @throws IOException
	 */
	public static void printMatrix(boolean matrix[][], String caminho) throws IOException {
		File f = new File(caminho);
		PrintWriter pw = new PrintWriter(f);
		...
		pw.close();
		System.out.println("Matriz de adjacencias gerada em disco com sucesso...");
	}
	...
	public static int getMaiorComponenteConexo(Grafo grafo, Dfs componenteConexo){
        ...
        int maiorComponente = componenteConexo.size(numeroComponentesConectados);
	...
	public static void getCaminhosBfs(Grafo grafo, String caminho, int vertice) throws IOException {
		File f = new File(caminho);
		PrintWriter pw = new PrintWriter(f);
		List<Double> distancias = new ArrayList<Double>();
		for(Double d : grafo.getDistancia(vertice))
		    distancias.add(d);
		pw.println("Caminho mais longo para o vertice "+ vertice +": "+grafo.getCaminhoMaisLongo(distancias));
	...
	public static void getDiametroGrafo(Grafo grafo){
		...
		System.out.println("Diametro do grafo: "+grafo.getCaminhoMaisLongo(lista));
		System.out.println("O diametro da internet foi calculado em: " + (System.currentTimeMillis() - tempoInicial) / 1000+"ms");
	}
	...
	public static void geraTxtPesos(Grafo grafo, String caminho) {
		...
		try {
			...
			pw.close();
			System.out.println("Matriz de pesos gerada em disco com sucesso...");
		} catch (FileNotFoundException e) {
			e.printStackTrace();
		}
	}
